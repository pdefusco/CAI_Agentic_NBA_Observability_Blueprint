...
function requireElements() {
    ...
    return {
        ...
        customerSelect: byId("customer-select"),
...
function formatCustomerLabel(name, tier, income, id) {
...
function showCustomerSkeleton(el) {
    el.customerSelect.style.display = "none";
    el.customerSelect.insertAdjacentHTML("afterend", '<div id="customer-skeleton" class="skeleton skeleton--select"></div>');
    el.input.disabled = true;
    el.send.disabled = true;
}
function clearCustomerSkeleton(el) {
    document.getElementById("customer-skeleton")?.remove();
    el.customerSelect.style.display = "";
}
function populateCustomers(el) {
    ...
    for (const customer of state.customers) {
        ...
        option.textContent = formatCustomerLabel(customer.full_name, customer.risk_tier, customer.annual_income, customer.customer_id);
        el.customerSelect.appendChild(option);
    }
...
function updateCustomerHint(el) {
    const customer = state.customers.find((c) => c.customer_id === state.customerId);
    if (!customer) {
        return;
    }
...
function updateScrollButton(el) {
    const distance = el.chat.scrollHeight - el.chat.scrollTop - el.chat.clientHeight;
    el.scrollBottom.classList.toggle("scroll-bottom--visible", distance > 120);
}
async function submitTurn(el, message) {
...
async function boot() {
    ...
    try {
        ...
        showToast(`Could not reach the API: ${err instanceof Error ? err.message : String(err)}`, "error");
    }
