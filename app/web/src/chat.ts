...
import { state, type ChatMessage } from "./state.js";
...
function feedbackButton(
  ...
  button.addEventListener("click", () => {
    ...
    void postFeedback(runId, score)
      ...
      .catch((err: unknown) => {
        showToast(
          `LangSmith feedback failed: ${err instanceof Error ? err.message : String(err)}`,
          "error",
        );
      });
  });
...
function typingElement(): HTMLElement {
  const wrapper = document.createElement("div");
 ...
 */
function emptyState(onSuggestion: (text: string) => void): HTMLElement {
  const wrapper = document.createElement("div");
