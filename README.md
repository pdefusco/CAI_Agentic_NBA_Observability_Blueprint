...
- idempotently seed `nba_demo.db` with the offer catalog and 10,000 synthetic customers (via `db.py` + `pii_datagen.py`);
...
With `LANGSMITH_API_KEY`, `LANGSMITH_PROJECT`, and the LLM/XGBoost endpoint env vars set, run these notebooks **in order**:
...
3. **`nba_experiments.ipynb`** — replays every scripted conversation turn-by-turn against the compiled LangGraph and grades the final state. Runs several experiments side-by-side so you can compare temperature and split cuts.
